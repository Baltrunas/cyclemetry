...
class Frame:
    ...
    def draw_elevation(self, img, config, activity, fps=None):
        ...
        # x = int(x * config["width"] / (len(heights) - 1))
        ...
        img.paste(elevation_img, (config["x"], config["y"]))
        return img

    def draw_course(self, img, config, activity, fps=None):
        ...
        current_x = int((current_lng - min_lng) * scale_factor)
        current_y = int((max_lat - min_lat) * scale_factor) - int((current_lat - min_lat) * scale_factor)
        draw.ellipse([(current_x - 30, current_y - 30), (current_x + 30, current_y + 30)], fill='#ffffff55')
        draw.ellipse([(current_x - 20, current_y - 20), (current_x + 20, current_y + 20)], fill='#ffffff88')
        ...
        # elevation_img = elevation_img.rotate(
        #     -90, resample=Image.Resampling.BICUBIC, expand=True
        # )
        ...
        for attribute in attributes:
            ...
            if "hide" not in config.keys() or not config["hide"]:
                ...
                if attribute == conf.ATTR_COURSE:
                    ...
                    img = self.draw_elevation(
                        img,
                        config["profile"],
                        self.activity,
                        fps=configs["scene"]["fps"],
                    )
                    img = self.draw_value(img, value, config["elevation"])
